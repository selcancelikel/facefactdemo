...
def get_ingredients():
    try:
        Blogs = Blog.query.all()  # Tum Not nesnelerini alin
        Blog_data = [Blog.data for Blog in Blogs]  # Her bir Not nesnesinin `data` ozelligini alin
        return render_template("icerik.html", Blogs=Blog_data)  # `Blog_data` listesini render_template'e gecirin
